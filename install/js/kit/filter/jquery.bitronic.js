...
$(document).ready(function(){
	...
	$.kitInherit(
		...
		$.kitSmartFilter,
		{
			...
			init: function(wrapper, options){
				...
				if(bSef)
					window.kitSefUrl = this.getSefUrl();
				else
					window.kitSefUrl = '';
				...
				if(this.isBitronic)
				{
					...
					this.form.on('click', '.kit-link .lvl2 a', function() {
						var $this = $(this);
						...
						if($this.data('checked') == 'checked'){
							$this.data('checked', '');
							lvl2.removeClass('kit-checked');
						}
						else{
							$this.data('checked', 'checked');
							lvl2.addClass('kit-checked');
						}
			...
			initHints: function()
			{
				...
				if(jQuery().tooltip) {
					$('.kit-filter-property-hint', _this.wrapper).each(function(){
						var $this = $(this);
						var hint = $this.next();
						if(hint.length)
						{
					...
					$('.kit-filter-property-hint', _this.wrapper).tooltip({
						position: {
							my: "center bottom-5",
							at: "center top",
						},
				...
				{
					$.KitSmartFilter.prototype.initHints.call(this);
				}
			},
			...
			submitForm: function(){
				...
				if(this.isBitronic)
				{
					...
					if(form.data('clicked') == 'del_filter')
					{
						...
						form.find('.kit-link .lvl2 a').each(function(){
							var link = $(this);
						...
						form.find('.kit-range div').ionRangeSlider("reset");
					}
				...
				{
					return $.KitSmartFilter.prototype.submitForm.call(this);
				}
			},
			...
			catalogLoading: function(values) {
				...
				$.post(_this.options.cfajaxURL, params, function(data) {
					...
					var regFilter = /<\!--START KIT_SMART_FILTER-->([\s\S]*?)<\!--END KIT_SMART_FILTER-->/gim;
			...
			postHandler: function(result){
				...
				$.KitSmartFilter.prototype.postHandler.call(this, result);
			...
			showModef: function(result){
				if(this.options.ajax_enable == "N") 
				{
					$.KitSmartFilter.prototype.showModef.call(this, result);
				}
			}
		}
	);
	...
	$.kitInherit(
		'kitBitronicHorizontalSmartFilter',
		$.kitBitronicSmartFilter,
		{
			catalogLoading: function(values) {
				var _this = this;
