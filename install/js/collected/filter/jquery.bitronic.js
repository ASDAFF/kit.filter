...
$(document).ready(function(){
	...
	$.collectedInherit(
		...
		$.collectedSmartFilter, 
		{
			...
			init: function(wrapper, options){
				...
				if(bSef)
					window.collectedSefUrl = this.getSefUrl();
				else
					window.collectedSefUrl = '';
				...
				if(this.isBitronic)
				{
					...
					this.form.on('click', '.collected-link .lvl2 a', function() {
						var $this = $(this);
						...
						if($this.data('checked') == 'checked'){
							$this.data('checked', '');
							lvl2.removeClass('collected-checked');
						}
						else{
							$this.data('checked', 'checked');
							lvl2.addClass('collected-checked');
						}
			...
			initHints: function()
			{
				...
				if(jQuery().tooltip) {
					$('.collected-filter-property-hint', _this.wrapper).each(function(){
						var $this = $(this);
						var hint = $this.next();
						if(hint.length)
						{
					...
					$('.collected-filter-property-hint', _this.wrapper).tooltip({
						position: {
							my: "center bottom-5",
							at: "center top",
						},
				...
				{
					$.CollectedSmartFilter.prototype.initHints.call(this);
				}
			},
			...
			submitForm: function(){
				...
				if(this.isBitronic)
				{
					...
					if(form.data('clicked') == 'del_filter')
					{
						...
						form.find('.collected-link .lvl2 a').each(function(){
							var link = $(this);
						...
						form.find('.collected-range div').ionRangeSlider("reset");
					}
				...
				{
					return $.CollectedSmartFilter.prototype.submitForm.call(this);
				}
			},
			...
			catalogLoading: function(values) {
				...
				$.post(_this.options.cfajaxURL, params, function(data) {
					...
					var regFilter = /<\!--START COLLECTED_SMART_FILTER-->([\s\S]*?)<\!--END COLLECTED_SMART_FILTER-->/gim;
			...
			postHandler: function(result){
				...
				$.CollectedSmartFilter.prototype.postHandler.call(this, result);
			...
			showModef: function(result){
				if(this.options.ajax_enable == "N") 
				{
					$.CollectedSmartFilter.prototype.showModef.call(this, result);
				}
			}
		}
	);
	...
	$.collectedInherit(
		'collectedBitronicHorizontalSmartFilter', 
		$.collectedBitronicSmartFilter, 
		{
			catalogLoading: function(values) {
				var _this = this;
