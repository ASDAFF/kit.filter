	...
	<?foreach($arResult["ITEMS"] as $arItem):?>
		<?if($priceCounter && isset($arItem["PRICE"])):?>
			<?if(isset($arItem["VALUES"]["MIN"]["VALUE"]) && isset($arItem["VALUES"]["MAX"]["VALUE"])):?>
				...
				<rangeFilter captionFrom="<?echo GetMessage("KOMBOX_CMP_FILTER_FROM")?>" captionTo="<?echo GetMessage("KOMBOX_CMP_FILTER_TO")?>" min="<?echo floor($arItem["VALUES"]["MIN"]["VALUE"])?>" max="<?echo ceil($arItem["VALUES"]["MAX"]["VALUE"])?>" step="1" <?if (count($arItem["CURRENCIES"]) == 1) echo ' unit="'.current($arItem["CURRENCIES"]).'"';?>>
