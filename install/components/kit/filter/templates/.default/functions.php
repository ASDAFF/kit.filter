		...
		switch($arItem['SETTINGS']['VIEW'])
		{
			...
			?>
				<div class="kit-num kit-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					...
					?>
					<?echo GetMessage("KIT_CMP_FILTER_FROM")?>
					<input 
			...
			case 'CHECKBOX':
			?>
				<div class="kit-combo kit-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>"<?if($arItem["CLOSED"]):?> style="display:none;"<?endif;?>>
					<?foreach($arItem["VALUES"] as $val => $ar):?>
			...
			case 'RADIO':
			?>
				<div class="kit-radio kit-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>"<?if($arItem["CLOSED"]):?> style="display:none;"<?endif;?>>
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						<?kitOtherValues($arItem, 'start');?>
						<div class="lvl2<?echo $ar["DISABLED"]? ' kit-disabled': ''?><?echo $ar["CHECKED"]? ' kit-checked': ''?>">
							<input
			...
			case 'LINK':
			?>
				<div class="kit-link kit-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>"<?if($arItem["CLOSED"]):?> style="display:none;"<?endif;?>>
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						<?kitOtherValues($arItem, 'start');?>
						<div class="lvl2<?echo $ar["DISABLED"]? ' kit-disabled': ''?><?echo $ar["CHECKED"]? ' kit-checked': ''?>">
