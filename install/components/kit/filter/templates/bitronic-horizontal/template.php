...
$APPLICATION->AddHeadScript("/bitrix/js/kit/filter/jquery.bitronic.js");
?>

<div id="ys_filter_bitronic" class="item_filters kit-filter">
	<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get"<?if($arResult['IS_SEF']):?> data-sef="yes"<?endif;?> class="smartfilter">
		...
		<table width="100%">
			...
			<?endif;?>
				<td width="33%">
					<div class="lvl1<?if($arItem["CLOSED"]):?> kit-closed<?endif;?> ye_option" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
						<div class="ya_pole">
							<div class="ye_head">
								<span class="ys-opt <?if($arItem["CLOSED"]):?>ys-hide<?else:?>ys-show<?endif?>">
									<span class="kit-filter-property-name ye_tit"><?=$arItem["NAME"]?></span>
...
<script>
	$(function(){
		$('#ys_filter_bitronic').kitBitronicHorizontalSmartFilter({
