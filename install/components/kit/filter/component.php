...
{
	ShowError(GetMessage('KIT_CMP_FILTER_MODULE_NOT_INSTALLED'));
	return;
}

if(!CModule::IncludeModule('iblock'))
{
	ShowError(GetMessage('KIT_CMP_FILTER_IBLOCK_MODULE_NOT_INSTALLED'));
	return;
}

$arResult['IS_SEF'] = CKitFilter::IsSefMode($arParams['PAGE_URL']);
...
if($this->StartResultCache(false, array($arrFilter,($arParams['CACHE_GROUPS']? $USER->GetGroups(): false), $arResult['IS_SEF'])))
{
	...
	if(!empty($arResult['ITEMS']))
	{
		...
		$res = CKitFilterIBlock::GetPropertyValues($this->IBLOCK_ID, $arElementFilter, $arItems);
		...
		if(!empty($arIDs) && $this->SKU_IBLOCK_ID)
		{
			...
			if($arResult['SKU_PROPERTY_COUNT'] > 0)
			{
				...
				$res = CKitFilterIBlock::GetPropertyValues($this->SKU_IBLOCK_ID, $arSkuFilter, $arItemsSKU);
				while($el = $res->Fetch())
				{
					...
					if(isset($arResult['ITEMS'][$NAME]))
					{
						...
						if(strlen($price) && $arParams['CONVERT_CURRENCY'])
						{
							...
							if($currency != $arParams['CURRENCY_ID'])
								$price = CKitFilter::ConvertCurrency($price, $currency, $arParams['CURRENCY_ID']);
						}
						...
						if(!strlen($arResult['ITEMS'][$NAME]['SETTINGS']['SLIDER_UNITS']))
						{
							...
							if(strlen($currency))
							{
								$arResult['ITEMS'][$NAME]['SETTINGS']['SLIDER_UNITS'] = CKitFilter::getCurrencyFullName($currency);
							}
						}
...
foreach($arResult['ITEMS'] as $PID => &$arItem)
{
	if(isset($_COOKIE['kit-filter-closed-'.$arItem['CODE_ALT'].'-'.$arItem['ID']]))
	{
		if($_COOKIE['kit-filter-closed-'.$arItem['CODE_ALT'].'-'.$arItem['ID']] == 'true')
			$arItem['CLOSED'] = true;
		else
			$arItem['CLOSED'] = false;
	}
