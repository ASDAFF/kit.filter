...
class CCollectedCatalogFilter extends CBitrixComponent
{
	var $IBLOCK_ID = 0;
	var $SKU_IBLOCK_ID = 0;
	...
	public function getPriceItems()
	{
		...
		if (!empty($this->arParams['PRICE_CODE']))
		{
			if(CModule::IncludeModule('catalog'))
			{
				...
				{
					$units = CCollectedFilter::getCurrencyFullName($this->arParams['CURRENCY_ID']);
				}
				...
				case 'STORES':
					$items = array(
						'STORES' => array(
							...
							'CODE' => 'STORES',
							'CODE_ALT' => 'stores',
							'NAME' => GetMessage('COLLECTED_CMP_FILTER_STORES_NAME'),
				...
				case 'SECTIONS':
					$items = array(
						'SECTIONS' => array(
							...
							'CODE' => 'SECTIONS',
							'CODE_ALT' => 'sections',
							'NAME' => GetMessage('COLLECTED_CMP_FILTER_SECTIONS_NAME'),
				...
				case 'AVAILABLE':
					...
					$items = array(
						'AVAILABLE' => array(
							...
							'CODE' => 'AVAILABLE',
							'CODE_ALT' => 'available',
							'NAME' => GetMessage('COLLECTED_CMP_FILTER_AVAILABLE_NAME'),
				...
				case 'QUANTITY':
					$items = array(
						'QUANTITY' => array(
							...
							'CODE' => 'QUANTITY',
							'CODE_ALT' => 'quantity',
							'NAME' => GetMessage('COLLECTED_CMP_FILTER_QUANTITY_NAME'),
							'PROPERTY_TYPE' => 'N',
							'HINT' => '',
	...
	public function fillItemValues(&$resultItem, $arProperty, $cnt = false)
	{
		...
		case 'SECTIONS':
			...
			break;
		case 'AVAILABLE':
			$value = GetMessage('COLLECTED_CMP_FILTER_AVAILABLE_VALUE_'.$key);
			$sort = $value;
			$value_id = $key;
	...
	protected function getRequestBySef()
	{
		...
		{
			$requestURL = CCollectedFilter::GetCurPage(true);
							...
							if(isset($arItems[$paramName]))
							{
								...
								foreach($arParamParts as $value)
								{
									if(strlen($value))
									{
										...
										if(!defined('COLLECTED_FILTER_ERROR_404'))
											@define('COLLECTED_FILTER_ERROR_404', 'Y');
									}
								}
								...
								{
									if(!defined('COLLECTED_FILTER_ERROR_404') && !empty($arParamValues))
										@define('COLLECTED_FILTER_ERROR_404', 'Y');
								...
								if(!defined('COLLECTED_FILTER_ERROR_404') && !$bSet)
									@define('COLLECTED_FILTER_ERROR_404', 'Y');
							}
							elseif(!defined('COLLECTED_FILTER_ERROR_404'))
							{
								@define('COLLECTED_FILTER_ERROR_404', 'Y');
							}
						}
	...
	public function getUrl($url, $arRequest = array(), $addParams = array())
	{
		...
		if($this->arResult['IS_SEF'])
		{
			return CCollectedFilter::urlAddParams($sefURL, $addParams, array(
				'skip_empty' => true,
				'encode' => true,
			));
		}
		else
		{
			...
			return CCollectedFilter::urlAddParams($clearURL, $paramsToAdd + $addParams, array(
				'skip_empty' => true,
				'encode' => true,
			));
		}
	...
	{
		foreach (GetModuleEvents('collected.filter', 'OnSetFilterDefault', true) as $arEvent)
			...
			if(in_array($arItem['SETTINGS']['VIEW'], array('SELECT', 'LIST', 'RADIO')) && !empty($arItem['VALUES']))
			{
				array_unshift(
					...
					array(
						...
						'HTML_VALUE' => '',
						'HTML_VALUE_ALT' => '',
						'VALUE' => GetMessage('COLLECTED_CMP_FILTER_ITEM_EMPTY'),
						'SORT' => 0,
						'UPPER' => ToUpper(GetMessage('COLLECTED_CMP_FILTER_ITEM_EMPTY')),
						'CNT' => 0,
