...
$APPLICATION->AddHeadScript("/bitrix/js/collected/filter/jquery.bitronic.js");
?>

<div id="ys_filter_bitronic" class="item_filters collected-filter">
	<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get"<?if($arResult['IS_SEF']):?> data-sef="yes"<?endif;?> class="smartfilter">
		...
		<table width="100%">
			...
			<?endif;?>
				<td width="33%">
					<div class="lvl1<?if($arItem["CLOSED"]):?> collected-closed<?endif;?> ye_option" data-id="<?echo $arItem["CODE_ALT"].'-'.$arItem["ID"]?>">
						<div class="ya_pole">
							<div class="ye_head">
								<span class="ys-opt <?if($arItem["CLOSED"]):?>ys-hide<?else:?>ys-show<?endif?>">
									<span class="collected-filter-property-name ye_tit"><?=$arItem["NAME"]?></span>
...
<script>
	$(function(){
		$('#ys_filter_bitronic').collectedBitronicHorizontalSmartFilter({
