...
if(!function_exists('collectedShowField'))
{
	function collectedShowField($arItem)
	{
		switch($arItem['SETTINGS']['VIEW'])
		{
				...
				<div class="collected-num collected-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					<table>
						<tr>
							...
							?>
							<td class="collected-from">
								<span><?echo GetMessage("COLLECTED_CMP_FILTER_FROM")?></span>
								<input 
			...
			case 'SELECT':
			?>
				<div class="collected-select collected-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
			...
			case 'LIST':
			?>
				<div class="collected-list collected-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					<select 
			...
			case 'CHECKBOX':
			?>
				<div class="collected-combo collected-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					<?foreach($arItem["VALUES"] as $val => $ar):?>
			...
			case 'RADIO':
			?>
				<div class="collected-radio collected-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						<?collectedOtherValues($arItem, 'start');?>
						<div class="lvl2<?echo $ar["DISABLED"]? ' collected-disabled': ''?><?echo $ar["CHECKED"]? ' collected-checked': ''?>">
							<input
			...
			case 'LINK':
			?>
				<div class="collected-link collected-filter-property-body" data-name="<?=$arItem["CODE_ALT"]?>">
					<?foreach($arItem["VALUES"] as $val => $ar):?>
						<?collectedOtherValues($arItem, 'start');?>
						<div class="lvl2<?echo $ar["DISABLED"]? ' collected-disabled': ''?><?echo $ar["CHECKED"]? ' collected-checked': ''?>">
