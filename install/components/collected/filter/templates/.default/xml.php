...
if ($priceCounter > 0)
{
	...
	$rangeCounter = 1;//Range propery will be skipped
	$totalCounter = 5;//Overall properties count
}
else
{
	$priceCounter = 0;//When price is NOT showed
	$rangeCounter = 2;//We can show no more than one range propery
	$totalCounter = 5;//Overall properties count
}
	...
	<?foreach($arResult["ITEMS"] as $arItem):?>
		<?if($priceCounter && isset($arItem["PRICE"])):?>
			<?if(isset($arItem["VALUES"]["MIN"]["VALUE"]) && isset($arItem["VALUES"]["MAX"]["VALUE"])):?>
				...
				<rangeFilter captionFrom="<?echo GetMessage("COLLECTED_CMP_FILTER_FROM")?>" captionTo="<?echo GetMessage("COLLECTED_CMP_FILTER_TO")?>" min="<?echo floor($arItem["VALUES"]["MIN"]["VALUE"])?>" max="<?echo ceil($arItem["VALUES"]["MAX"]["VALUE"])?>" step="1" <?if (count($arItem["CURRENCIES"]) == 1) echo ' unit="'.current($arItem["CURRENCIES"]).'"';?>>
		...
		<?elseif($rangeCounter && $arItem["PROPERTY_TYPE"] == "N"):?>
			...
			<?endif;?>
		<?elseif($totalCounter && !empty($arItem["VALUES"]) && $arItem["PROPERTY_TYPE"] != "N"):;?>
			<?$totalCounter--;?>
			<dropDown>
				<description caption="<?echo $arItem["NAME"]?>">
					<setParameter name="<?echo $arItem["CODE_ALT"]?>"/>
				</description>
				<?foreach($arItem["VALUES"] as $val => $ar):?>
					<dropDownValue key="<?echo $ar["CONTROL_NAME_ALT"]?>" caption="<?echo $ar["VALUE"];?>"/>
				<?endforeach;?>
			</dropDown>
		<?endif;?>
	<?endforeach;?>
