...
{
	ShowError(GetMessage('KOMBOX_CMP_FILTER_IBLOCK_MODULE_NOT_INSTALLED'));
	return;
}

$arResult['IS_SEF'] = CKomboxFilter::IsSefMode($arParams['PAGE_URL']);
...
if($this->StartResultCache(false, array($arrFilter,($arParams['CACHE_GROUPS']? $USER->GetGroups(): false), $arResult['IS_SEF'])))
{
	...
	if(!empty($arResult['ITEMS']))
	{
		...
		$res = CKomboxFilterIBlock::GetPropertyValues($this->IBLOCK_ID, $arElementFilter, $arItems);
		...
		if(!empty($arIDs) && $this->SKU_IBLOCK_ID)
		{
			...
			if($arResult['SKU_PROPERTY_COUNT'] > 0)
			{
				...
				$res = CKomboxFilterIBlock::GetPropertyValues($this->SKU_IBLOCK_ID, $arSkuFilter, $arItemsSKU);
				while($el = $res->Fetch())
				{
					...
					if(isset($arResult['ITEMS'][$NAME]))
					{
						...
						if(strlen($price) && $arParams['CONVERT_CURRENCY'])
						{
							...
							if($currency != $arParams['CURRENCY_ID'])
								$price = CKomboxFilter::ConvertCurrency($price, $currency, $arParams['CURRENCY_ID']);
						}
						...
						if(!strlen($arResult['ITEMS'][$NAME]['SETTINGS']['SLIDER_UNITS']))
						{
							...
							if(strlen($currency))
							{
								$arResult['ITEMS'][$NAME]['SETTINGS']['SLIDER_UNITS'] = CKomboxFilter::getCurrencyFullName($currency);
							}
						}
