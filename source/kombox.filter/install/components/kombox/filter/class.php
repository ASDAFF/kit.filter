...
class CKomboxCatalogFilter extends CBitrixComponent
{
	...
	public function getPriceItems()
	{
		...
		if (!empty($this->arParams['PRICE_CODE']))
		{
			if(CModule::IncludeModule('catalog'))
			{
				...
				{
					$units = CKomboxFilter::getCurrencyFullName($this->arParams['CURRENCY_ID']);
				}
				...
				case 'AVAILABLE':
					...
					$items = array(
						'AVAILABLE' => array(
							...
							'CODE' => 'AVAILABLE',
							'CODE_ALT' => 'available',
							'NAME' => GetMessage('KOMBOX_CMP_FILTER_AVAILABLE_NAME'),
				...
				case 'QUANTITY':
					$items = array(
						'QUANTITY' => array(
							...
							'CODE' => 'QUANTITY',
							'CODE_ALT' => 'quantity',
							'NAME' => GetMessage('KOMBOX_CMP_FILTER_QUANTITY_NAME'),
							'PROPERTY_TYPE' => 'N',
							'HINT' => '',
	...
	public function fillItemValues(&$resultItem, $arProperty, $cnt = false)
	{
		...
		case 'SECTIONS':
			...
			break;
		case 'AVAILABLE':
			$value = GetMessage('KOMBOX_CMP_FILTER_AVAILABLE_VALUE_'.$key);
			$sort = $value;
			$value_id = $key;
							...
							if(isset($arItems[$paramName]))
							{
								...
								foreach($arParamParts as $value)
								{
									if(strlen($value))
									{
										...
										if(!defined('KOMBOX_FILTER_ERROR_404'))
											@define('KOMBOX_FILTER_ERROR_404', 'Y');
									}
								}
								...
								{
									if(!defined('KOMBOX_FILTER_ERROR_404') && !empty($arParamValues))
										@define('KOMBOX_FILTER_ERROR_404', 'Y');
								...
								if(!defined('KOMBOX_FILTER_ERROR_404') && !$bSet)
									@define('KOMBOX_FILTER_ERROR_404', 'Y');
							}
							elseif(!defined('KOMBOX_FILTER_ERROR_404'))
							{
								@define('KOMBOX_FILTER_ERROR_404', 'Y');
							}
						}
	...
	public function getUrl($url, $arRequest = array(), $addParams = array())
	{
		...
		if($this->arResult['IS_SEF'])
		{
			return CKomboxFilter::urlAddParams($sefURL, $addParams, array(
				'skip_empty' => true,
				'encode' => true,
			));
		}
		else
		{
			...
			return CKomboxFilter::urlAddParams($clearURL, $paramsToAdd + $addParams, array(
				'skip_empty' => true,
				'encode' => true,
			));
		}
	...
	{
		foreach (GetModuleEvents('kombox.filter', 'OnSetFilterDefault', true) as $arEvent)
			...
			if(in_array($arItem['SETTINGS']['VIEW'], array('SELECT', 'LIST', 'RADIO')) && !empty($arItem['VALUES']))
			{
				array_unshift(
					...
					array(
						...
						'HTML_VALUE' => '',
						'HTML_VALUE_ALT' => '',
						'VALUE' => GetMessage('KOMBOX_CMP_FILTER_ITEM_EMPTY'),
						'SORT' => 0,
						'UPPER' => ToUpper(GetMessage('KOMBOX_CMP_FILTER_ITEM_EMPTY')),
						'CNT' => 0,
